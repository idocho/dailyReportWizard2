...
| 항목 | 경로 |
|------|------|
| **현재 개발 라인 (웹 수정은 여기만)** | `code/public/v2.2.4/` — JS `js/app-core.js`·`app-input.js`·`app-scores.js`·`app-settings.js`, CSS `css/app.css`, HTML `index.html` |
| 동결 버전 (수정 금지) | `code/public/v2.2.3/` 이하 전 버전 — 릴리즈 시 `scripts/new-version.ps1`로 동결 복제 |
...
| 커리큘럼 원본 JSON | `code/public/v2.2.4/data/math-curriculum-2022.json` (버전 디렉터리 내 — 별도 `src/` 없음) |
