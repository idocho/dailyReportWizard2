...
function setAdminMode(on){
  ...
  on=!!on; if(adminOn===on)return;
  adminOn=on;
  if(!on && curAI>=(instructor?.assignments||[]).length)curAI=0; // 관리자 해제 시 범위 가드
  renderSb(); renderMain();
}
...
function goNav(n){
  if(n==='bulk'&&activeTab!=='bulk')_bulkTabFresh=true;   // 다른 탭→일괄공지 진입 시에만 전체선택 리셋(탭 내부 재렌더와 구분)
  activeTab=n;
  ...
  sb.innerHTML=`
    <div class="sb-me" onclick="showIM()">
      <div style="display:flex;align-items:center;gap:8px">
        <div class="avatar"${adminOn?' style="background:#D97706"':''}>${esc((instructor?.name||'?').slice(0,3))}</div>
    ...
    <div class="md-hint"${adminOn?' style="color:#FBBF24"':''}>${adminOn?'⚡ 전체 수업 · 교무 권한 ON':'내 담당 수업만 표시'}</div>`:''}
    <div class="sb-nav">
