...
const AGENT_DL = 'https://github.com/idocho/dailyReportWizard2/releases/download/agent/DRW-AI-Agent-0.100.exe';
let _rpPending = null;
...
async function openReportSend(force){
  ...
  _rpModal(`<h3>카카오톡 전송 — ${groups.length}반 · ${totalReady}명</h3>
    <div class="rp-hint">검토중(발송문 작성)인 학생만 표시 — 제외할 반/학생 체크 해제. 전체 선택 학생이 하나의 큐로 순차 발송됩니다.</div>
...
async function doReportSend(){
  ...
  // 반 단위 관리 불필요(사용자 확정) — 선택된 전체 대상을 반 무관 단일 job으로 묶어
  // 하나의 큐로 순차 발송. 반별 학생 조회만 캐시(_buildMessage엔 classId 필요).
  ...
  for(const [classId, nks] of Object.entries(byCls)){
    ...
    nks.forEach(nk => {
      ...
      recipients.push({ nameKey: nk, name: s.name || nk, note, msg: _buildMessage(classId, nk, s.name || nk, note), status: '대기' });
    });
  }
  if(!recipients.length) return;
  const clsSet = Object.keys(byCls);
  const cls = clsSet.length > 1 ? `${clsSet.length}개 반` : (clsSet[0] || '');
  const jid = Date.now() + '_' + Math.floor(Math.random() * 100000);
  try{
    await fbPut(`sendJobs/${instructor.id}/${jid}`, { cls, recipients, status: 'queued', ts: Date.now(), date: todayKey(), instructor: instructor.id });
    toast(`전송 큐 적재: ${cls} · ${recipients.length}명 — 에이전트가 순차 발송`);
  }catch(e){ toast(`전송 요청 실패 — ${e.message || e}`); }
  loadReportJobs();
}

// ── 전송 상태 ─────────────────────────────────────────────────────────
let _rpJobDetailOpen = new Set();   // 상세 펼침 상태 — 2.5s 폴링 재렌더에도 유지
...
async function loadReportJobs(){
  ...
  el.innerHTML = (arr.length ? arr.map(([id, j]) => {
    ...
    // 실패 상세 — job 레벨 오류(j.error) + 건별 실패 사유(recipients[i].error). 지금까지
    // Firebase 원본에만 있고 화면엔 카운트만 나오던 것 → 펼치기로 텍스트 노출.
    const failRecs = recs.filter(r => r.status === '실패' && r.error);
    const hasDetail = !!j.error || failRecs.length > 0;
    const open = _rpJobDetailOpen.has(id);
    const detailBtn = hasDetail
      ? `<a class="rp-jx-d" onclick="toggleJobDetail('${esc(id)}')">${open ? '상세 숨기기' : '상세'}</a>` : '';
    const detailBody = (hasDetail && open) ? `<div class="rp-job-detail">${
        (j.error ? `<div>⚠ ${esc(j.error)}</div>` : '')
        + failRecs.map(r => `<div>· ${esc(r.name || r.nameKey || '')} — ${esc(r.error)}</div>`).join('')
      }</div>` : '';
    return `<div class="rp-job">
      ...
      <div class="rp-job-sub"><span>${done + err}/${tot}${exc ? ` · 제외 ${exc}` : ''}${err ? ` · 실패 ${err}` : ''}</span><span>${detailBtn}${cancelBtn}</span></div>
      ${detailBody}
    </div>`;
