      ...
      <div class="si-right">
        <div class="si-row">
          ...
          <div style="min-width:0">
            ...
            <input class="inp sm" value="${esc(tags.assign_note||'')}" placeholder="직접 입력" data-classid="${esc(classId)}" data-namekey="${esc(nameKey)}" data-subject="${esc(subject)}" oninput="onAssignNote(this)" style="width:100%;margin-top:6px">
          </div>
        </div>
...
// ── 과제 수행도 직접 입력(자유 문구, 최우선 반영) ────────────────────
// assign_grade(버튼)와는 별개 필드(assign_note)에 저장. app-report.js의
// _rpData()가 메시지 조립 시 assign_note가 있으면 버튼 등급 라벨 대신 이 텍스트를
// "▶ 과제 수행도" 섹션·AI 프롬프트(items[].value)에 그대로 사용(우선순위 최상단).
// 성취도 통계(analyzer)가 참조하는 assign_grade 자체는 그대로 유지·영향 없음.
function onAssignNote(el){
  const {classid:classId,namekey:nameKey,subject}=el.dataset;
  const tags=getTags(classId,nameKey,subject);
  tags.assign_note=el.value;
  pushObs(classId,nameKey,subject,'assign_note');
}
