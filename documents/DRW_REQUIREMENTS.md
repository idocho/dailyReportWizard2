...
**문서 버전**: 9.26 · **앱 버전**: v2.5.1(정식) · **최종 수정**: 2026-09-23
...
| 문서 버전 | 날짜 | 주요 변경 |
|-----------|------|-----------|
| 9.26 | 2026-09-23 | **[릴리스] 사용자 요청에 따라 에이전트 버전 0.99로 통일**. 이미지 창 유지 수정은 보존하고 GUI·빌드명·공개 다운로드 링크·포털의 현재 버전만 0.99로 변경. 0.99 공개 실행 파일을 수정본으로 교체. app-report.js 캐시 v338. 과거 0.100 릴리스 이력은 보존. |
| 9.25 | 2026-09-23 | **[릴리스] 에이전트 0.100 공개 배포 구성**. GitHub agent 릴리스에 0.100 exe 추가, 웹 가이드·에이전트 안내 다운로드 링크 갱신. app-report.js 캐시 v337, 버전 포털 설명 동기화. 이미지 포함 방 유지·종료 2초 대기 제거 포함. |
| 9.24 | 2026-09-23 | **[에이전트 0.100] 이미지 창 유지 수정 빌드**. GUI 버전·빌드 산출명 0.100으로 갱신해 0.99와 구분. -Clean 없이 기존 exe·설정 보존. 빌드 스크립트는 PyInstaller 실패 시 완료 메시지 대신 오류 반환. 로컬 빌드이며 공개 다운로드 링크·배포는 별도. |
| 9.23 | 2026-09-23 | **[에이전트] 이미지 전송 창 유지**. 이미지 단독·텍스트 혼합 모두 건별 및 전체 종료 시 창 닫기 제외. 동일 배치의 이미지 대상 방은 텍스트 건·오류·취소 후에도 정리 대상에서 제외. 이미지 확인 팝업 시간초과 시 Esc 생략. 전체 종료의 고정 2초 대기 제거; 카톡 이미지 확인 절차 후 다음 수신자로 진행. 에이전트 재빌드·교체 필요. |
| 9.22 | 2026-09-23 | **[웹] 일괄 공지 이미지 단독 전송 허용**. 메시지가 공백이어도 이미지가 있으면 수신자 msg를 빈 문자열로 유지하고 image를 포함한 sendJob 생성. 텍스트·이미지 모두 없으면 차단. 수신자 선택·에이전트 확인은 유지. 기존 에이전트는 빈 텍스트를 건너뛰고 이미지를 전송하므로 변경 불필요. app-report.js 캐시 v336. |
| 9.21 | 2026-09-22 | **[웹] 전송 모니터 완료→오류 경합 수정**. 자동 정리는 최신 작업을 ETag로 재조회하고 조건부 PUT으로만 갱신. 에이전트 완료·취소·삭제 및 수신자 변경과 경합 시 412 후 최신 상태 표시. 하트비트 조회 실패는 종료로 판단하지 않음. sending 또는 기존 자동 정리 오류는 수신자 전원 완료가 확인되면 done 복구; 실제 오류·부분 완료는 완료로 덮지 않음. app-report.js 캐시 v335. |
| 9.20 | 2026-09-21 | **[웹] 메시지 이름의 동명이인 식별 숫자 제거**. v2.5.1 메시지 조립 시 이름 끝 연속 숫자를 제거한 후 성 제외·받침 조사를 적용(`이도현8` → `▶ 오늘의 도현이는?`). 일괄 공지 `{이름}`도 `이도현`으로 치환. 원본 학생명·nameKey·전송 수신자 이름·카톡방 검색·AI 응답 매칭은 유지. 캐시버스트 app-report.js v334. Firebase Hosting 운영 배포 완료(2026-09-21); 운영 index.html·app-report.js 수정본 일치 및 루트 v2.5.1 연결 확인. |
| 9.19 | 2026-08-18 | **[웹] 9.18 과제 수행도 직접입력 텍스트박스 잘림 수정**. `.si-row`가 `grid-template-columns:40px 1fr`(라벨 40px·내용 1fr) 2컬럼 그리드인데, "과제" 행에 `si-lbl`·`si-btns`·`input` **3개**의 직계 자식을 넣어 CSS grid 자동배치상 3번째 자식(input)이 다음 줄 **첫 컬럼(40px 라벨 칸)**으로 밀려 들어가 입력창이 약 2글자 폭으로 잘려 보이던 버그(실사용 스크린샷으로 확인 — "과지" 두 글자만 노출). `si-btns`와 `input`을 `<div style="min-width:0">` 래퍼 하나로 묶어 `si-row`의 자식을 라벨+래퍼 2개로 되돌림(다른 모든 행과 동일 패턴) — 이제 래퍼가 1fr 칸 전체를 차지하고 그 안에서 버튼줄·입력창이 세로로 쌓임. 플레이스홀더도 장문("과제 수행도 직접 입력 — 입력 시 메시지에 최우선 반영(미입력 시 왼쪽 등급 사용)")→"직접 입력 시 최우선 반영"으로 축약(라벨이 이미 "과제"를 보여주므로 중복 불필요, 좁은 모바일 폭에서도 안 잘림). **검증**: 실제 스크립트를 390px 폭 컨테이너에 렌더해 `getBoundingClientRect()`로 실측 — 수정 전 컬럼폭 40px에 갇히던 입력창이 수정 후 콘텐츠 컬럼 폭(206px) 전체를 정상 차지·라벨 바로 옆(left=156.8px)에서 시작함을 확인. 캐시버스트 `app-input.js`(v333→v334), `firebase deploy --only hosting` 배포·라이브 반영 확인. |
| 9.18 | 2026-08-18 | **[웹] 9.17 재설계 — 직접입력이 실제 발송 메시지·AI 프롬프트의 "과제 수행도"를 우선 대체하도록 수정**. 실사용 피드백: 9.17의 `assign_tags` 칩 방식은 AI 노트 생성용 "과제 관련 특이사항" 버킷에만 얹혀 최종 메시지의 **구조화된 "▶ 과제 수행도" 섹션**(`app-report.js` `_rpData()`가 `assign_grade`→`_assignText()`로만 채움, `items[].value`로 AI 프롬프트 "수행도=" 라인까지 이어지는 별도 채널)엔 전혀 닿지 않아 "반영이 안 된다"는 문제 재발 + UI도 "버튼 추가" 형태라 부적합. 재설계: ①**UI를 칩/버튼에서 순수 텍스트 입력으로 교체** — `toggleCustomAssignInput`/`addCustomAssignTag`/`removeCustomAssignTag`/`onCustomAssignKey`·`.tg-custom-add`/`.tg-custom-chip`/`.custom-inp-row` 전부 제거, "과제" 행에 메모 인풋과 동일한 스타일의 상시 노출 텍스트박스(placeholder로 우선순위 명시) + `onAssignNote` 단일 핸들러로 교체(신규 필드 `assign_note`, `obs/{nk}/{subject}/{date}`에 `pushObs`로 저장 — `assign_grade`와 별개 필드라 analyzer 통계 불변). ②**`_rpData()`가 `assign_note` 존재 시 `_assignText(assign_grade)` 대신 그 문구를 `assignMap[sub]`으로 사용**(직접입력 우선, 미입력 시 버튼 등급 라벨 폴백) — 이 값이 그대로 최종 메시지 "▶ 과제 수행도" 섹션과 `_genCtx()`의 `items[].value`(→ai_engine.py "수행도=" 라인)에 흘러 **에이전트 코드 변경 없이** AI 노트·구조화 섹션 양쪽에 확정 반영됨. `_genCtx()`의 generic tags 병합에서 `assign_note`도 `assign_grade`처럼 제외(이미 items 채널로 전달되므로 특이사항 버킷 중복 방지). **검증**: 실제 프로덕션 스크립트(app-core+app-input+app-report) 3파일을 `<script>` 주입해 (a) 칩 UI 완전 제거 확인 (b) `assign_grade`만 설정 시 등급 라벨 폴백 (c) `assign_note` 동시 설정 시 그 문구로 대체 (d) `_buildMessage()` 최종 텍스트에 "▶ 과제 수행도" 섹션이 직접입력 문구를 포함 (e) `_genCtx()`의 `items[].value`도 동일 문구, `tags`엔 `assign_grade`/`assign_note` 미포함 — 전부 실코드 경로로 확인. 캐시버스트 `app-input.js`(v332→v333)·`app-report.js`(v331→v333)·`app.css`(20260642→20260643), `firebase deploy --only hosting` 배포·라이브 반영 확인. 에이전트 변경 없음(기존 0.99 유지, DL 200). |
| 9.17 | 2026-08-18 | **[웹] 과제 수행도 — 직접 입력(자유 문구) 옵션 추가**. 수업 입력 탭 "과제" 행에 고정 등급(✅완료~❌미완, `assign_grade`, analyzer 5축 통계가 참조하는 enum이라 불변)·강사 등록 프리셋(복수선택)에 이어 **`✏️ 직접입력`** 트리거 신설 — 클릭 시 인라인 텍스트박스 노출, 입력(Enter/추가 버튼) 시 제거 가능한 칩으로 추가(중복·빈값 무시), × 로 개별 삭제. 저장 위치는 기존 프리셋과 **동일한 `assign_tags`**(자유 문구 배열)라 AI 리포트 생성(`ai_engine._build_tags_context`가 이미 "과제 관련 특이사항"으로 그대로 반영)엔 코드 변경 없이 자동 연동되고, `assign_grade` enum·analyzer 통계엔 영향 없음(문서 §과제수행도 원칙 유지). 목업(`documents/mockups/assign-grade-freetext.html`) 선승인 후 실코드 반영(CLAUDE.md 목업 우선 원칙 준수). 구현: `app-input.js`에 `toggleCustomAssignInput`·`addCustomAssignTag`·`removeCustomAssignTag`·`onCustomAssignKey` 신설(기존 `onAssignTag`와 동일 patt., `getTags`/`pushObs` 재사용), `app.css`에 `.tg-custom-add`·`.tg-custom-chip`·`.custom-inp-row` 등 신규 스타일. **검증**: 로그인 우회 없이 실제 프로덕션 스크립트(app-core.js+app-input.js)를 `<script>` 주입으로 로드해 `renderInput()` 실호출 — 렌더(칩 표시)·추가·중복 방지·빈값 방지·삭제까지 전부 실코드 경로로 확인(스텁은 Firebase 쓰기(`pushObs`)만). `code/public/v2.5.1/index.html` 캐시버스트 `app-input.js`(v324→v332)·`app.css`(20260641→20260642). `firebase deploy --only hosting` 배포, 라이브 반영(신규 함수·클래스명·캐시버스트) 실측 확인. 에이전트: 현재 릴리스된 0.99.exe가 이미 최신 소스 기준으로 빌드·업로드된 상태(로컬-릴리스 SHA256 일치 확인) — 이번 웹 변경엔 에이전트 재빌드 불필요, 다운로드 링크(`app-report.js`·`guide.html`) 0.99 유지·DL 200 확인. |
| 9.16 | 2026-08-18 | **[에이전트 0.99] `NameError: name 'dups' is not defined` — 실발송 성공 건이 전부 job.status="오류"로 뒤집히던 버그 수정**. 9.10에서 동명이인 가드를 job-국소 `dups`(Counter)→배치 전체 `global_dups`(nameKey 기준)로 리팩터하며 변수명을 바꿨는데, `process_sendjobs` 맨 끝 최종 상태 patch 줄(`"excluded": len(dups)`)에 옛 변수명이 남아있었음 — 모든 수신자 발송이 끝난 **뒤** 이 줄에서 매번 NameError 발생 → 상위 `except`가 잡아 job.status를 "오류"로 덮어씀(개별 수신자는 이미 "완료"로 정상 기록된 상태). 9.14의 전송 모니터 "상세" 펼치기가 이 오류 텍스트를 처음 노출시켜 발견(실사용 테스트 리포트: "전송은 성공했는데 상태는 오류"). `excluded = len(recs) - len(send_idx)`로 수정, 목업 아닌 **`process_sendjobs` 전체 실행 경로를 fake DB로 end-to-end 실행**해 정상 종료·`excluded` 값·`status=done` 확인(이전엔 `py_compile`과 dedup 로직 단위 테스트만 해서 이 런타임 전용 버그를 놓쳤음). 에이전트 0.99 재빌드·릴리스, 웹 다운로드 링크 동기화·재배포. |
| 9.15 | 2026-08-18 | **[웹] 일괄 공지 트리 접기/펼치기 시 선택 초기화 버그 수정**. `renderBulk`가 호출될 때마다(진입 재렌더 포함) `_bulkSel`을 무조건 전체선택으로 리셋 → 트리 반 헤더의 접기/펼치기(`toggleTreeCls`→`renderMain()`→`renderBulk` 재호출)가 그 즉시 사용자가 해제해 둔 선택을 **전체선택으로 되돌리던 버그**(펼침·접힘 둘 다 동일 발생, 선택 해제한 반 외 다른 반까지 전부 재선택됨). `_bulkTabFresh` 플래그 도입(`goNav`가 다른 탭→`bulk` **최초 진입** 시에만 true 세팅) — `renderBulk`는 이 플래그가 true일 때만 전체선택으로 리셋하고 소비, 트리 폴드 토글·이미지 첨부·템플릿 삭제 등 **탭 내부 재렌더에서는 기존 선택을 유지**(무효화된 nameKey만 정리). 웹 v331. |
| 9.14 | 2026-08-18 | **[릴리스] v2.5.1 버전컷 + 전송 모니터 실패 상세 + 에이전트 하트비트 오탐 수정(에이전트 0.98)**. ① `scripts/new-version.ps1`로 v2.5.0 동결·`v2.5.1/` 신설(스크립트 자체도 `document.title` 미치환 버그 수정), `versions.json`·`firebase.json`(v2.5.0 ignore+redirect 편입, 루트 302를 v2.5.1로) 갱신 후 `build-portal.ps1`·`firebase deploy --only hosting` 배포, 루트·구버전 리다이렉트·다운로드 링크 실측 확인. ② **전송 모니터 실패 상세 펼치기** — 9.13까지 실패 사유(`recipients[i].error`·`job.error`)가 Firebase 원본에만 있고 화면엔 카운트만 나오던 관찰가능성 공백 해소. job에 실패 있으면 "상세" 토글(`toggleJobDetail`, `_rpJobDetailOpen`으로 2.5s 폴링 재렌더에도 펼침 상태 유지) → 건별 이름+오류 텍스트 나열. ③ **에이전트 하트비트 오탐 수정(실사용 테스트 중 발견 — "전송 성공했는데 상태는 오류")**: 하트비트가 `agent_gui.py` 메인루프 최상단(15s 주기)에서만 갱신되는데 그 루프는 `process_sendjobs` 블로킹 동안 못 돎 → 발송이 90초 이상 걸리면(다건·재시도 누적) 실제로는 살아있는데도 9.12의 유령작업 자동정리(`loadReportJobs`)가 하트비트 만료로 오판해 방금 성공한 건까지 job.status를 "오류"로 뒤집는 사고. `process_sendjobs`가 수신자 완료 콜백마다(12s 스로틀) 직접 하트비트 갱신하도록 수정 — 에이전트 생존 여부가 job 처리 시간과 무관해짐. 에이전트 0.98 재빌드·릴리스(웹 다운로드 링크 동기화). **부수 사고 기록**: 이 과정에서 `build-agent.ps1 -Clean` 옵션이 `code/dist/`를 통째로 지워, 그 폴더에서 직접 실행 중이던(가이드 권고와 달리 별도 폴더로 안 옮기고) exe의 로컬 `agent_config.json`(캠퍼스·강사명·API키)이 유실됨 — 휴지통·VSS 복구 시도 실패(PowerShell `Remove-Item -Force`는 휴지통 미경유), 재설정으로 복구. 이후 `-Clean` 미사용 원칙화, `build-agent.ps1` 주석에 "exe는 dist 밖 고정 폴더에서 실행" 경고 추가(CLAUDE.md 동기화). |
| 9.13 | 2026-08-18 | **[배포] 9.09~9.12 반영 — 에이전트 0.97 빌드·릴리스 + 웹 호스팅 배포**. 에이전트: `AGENT_VERSION`·`build-agent.ps1` 산출물명 0.96→**0.97**, PyInstaller 재빌드(20.8MB) 후 GitHub Release(`agent` 태그)에 신규 자산 업로드(기존 0.94·0.95 등 과거본은 유지, clobber 안 함 — 9.07 정책 계승). 웹 다운로드 링크(`app-report.js` `AGENT_DL`·`guide.html`)를 0.97로 동기화. **부수 발견**: 배포 전 릴리스 자산 조회 결과 0.96.exe가 애초에 업로드된 적 없어 — 직전까지 신규 강사 다운로드 버튼이 항상 404였음(9.07 문서화 당시 업로드 누락 추정, 아무도 신규 설치를 안 해 미발견). 이번 0.97 업로드로 해소. 웹: `index.html`의 `app-report.js` 캐시버스트 v327→v328, `firebase deploy --only hosting`으로 `dailyreportwizard.web.app` 라이브 반영 후 배포 페이지 실측 확인(가이드 다운로드 링크 0.97·접두사 폼 삭제 확인됨). |
| 9.12 | 2026-08-18 | **[웹] 리포트 전송 반별 job 분리 폐지 + 유령 전송 job 자동 정리**. ① `doReportSend`(`app-report.js`)가 선택 학생을 반별로 쪼개 job을 여러 개 생성하던 것을 **단일 job**(반 무관, 선택 전체를 한 큐로 순차 발송)으로 통합 — 반 단위 취소/모니터 분리 관리가 불필요하다는 확인에 따름(반별 학생 조회만 내부 캐시, `_buildMessage`엔 classId 필요해 유지). ② **전송 모니터 유령 작업 수정** — 에이전트 프로세스가 전송 도중 강제종료·PC 재부팅 등으로 죽으면 `agent_worker.py`의 완료 처리(`finally`)가 아예 못 돌아 Firebase job이 `status:'sending'`에 영구 고정, 취소를 눌러도 `cancel` 플래그를 읽어줄 프로세스가 없어 "중단 중…" 표시도 영구 고정 — 게다가 정리 버튼(`clearDoneJobs`)은 done/canceled/error만 지워서 삭제도 안 됨(신고된 유령 사례). `loadReportJobs()`가 폴링 시 `status:'sending'`인 job을 에이전트 하트비트(`agents/{id}.ts`, 90s 기준 `_agentAlive()`)와 대조 — 하트비트 죽었으면 죽은 job으로 간주해 `status:'error'`로 자동 전환, 기존 정리 버튼 대상에 자연 편입시킴. |
| 9.11 | 2026-08-18 | **[에이전트] 카톡 방 이름 전사 고정 포맷 반영 — 검색어 자체로 접두 컬리전 예방 + 강사별 설정 폼 폐지**. 실제 카톡 방 제목이 캠퍼스 무관 **"오직 {이름}! {브랜드}"로 전사 고정**됨을 확인 → 9.09는 사후 검증(`room_opened`)만 경계 인식했지 검색어 자체는 안 바뀌어, "오직 이건" 검색이 "오직 이건호! ..." 방을 후보에 올릴 위험이 여전(카톡 자체 검색 랭킹이 엉뚱한 방을 상위로 주면 재시도해도 같은 검색어라 못 벗어남 — 그 학생은 실패 반복). 검색어에 고정 접미 `"!"`까지 포함(`constants.KAKAO_ROOM_PREFIX`="오직 "·`KAKAO_ROOM_SUFFIX`="!")시켜 **카톡 검색 단계에서부터** 후보 배제("오직이건!"은 "오직이건호!..."의 부분문자열이 아님) — 실패 후 방어가 아니라 사전 예방. 강사별로 다르지 않은 전사 고정값이라 설정 메뉴의 "카톡 방 접두사" 입력 폼(`agent_gui.py`) 및 CLI 설정 프롬프트(`agent_worker.py _setup_cli`) **완전 제거**, `cfg.get("roomPrefix")` 참조도 제거하고 상수로 대체. 부수 수정: `_title_matches_room`(`kakao_send.py`)이 room_norm 끝이 이미 비-이름 문자(`!`)면 그 자체를 확정 경계로 보고 즉시 통과하도록 보강 — 안 하면 "!" 뒤에 오는 브랜드명("그릿PT", 첫 글자가 한글)이 "이름 이어짐"으로 오인되어 **정상 방까지 거부**하는 회귀가 생겼음(수정 중 자체 발견·즉시 교정). |
| 9.10 | 2026-08-18 | **[에이전트] 동명이인 가드 크로스-job 확장 + nameKey 판정으로 정밀화**. `process_sendjobs`(`agent_worker.py`)의 동명이인 오발송 가드가 **job 내부만** 검사했는데, 웹은 반별로 job을 분리 생성(`app-report.js` `doReportSend`) → 동명이인이 다른 반이면 서로 다른 job에 흩어져 가드 무력화(자동 발송됨). 배치(`pending` 전체) 수신자를 이름별로 모아 크로스-job까지 보되, **판정은 nameKey 기준**(이름별로 모인 서로 다른 nameKey 개수 > 1일 때만 동명이인)으로 변경 — 표시이름만으로 판정 시 발생 가능한 오탐(다른 nameKey 없이 같은 이름이 우연히 겹치는 경계 케이스)을 배제. (학생 ID 하나당 메시지 하나가 기본 원칙이라 한 학생이 여러 반 리포트로 중복 발송되는 케이스 자체는 발생하지 않음 — 검토 범위 아님.) (※ 이전에 이미 처리 완료된 job과의 중복까지는 미커버 — 같은 배치·같은 처리 사이클 한정) |
| 9.09 | 2026-08-18 | **[에이전트] 카톡 채팅방 검증 경계 인식 — 동명이인/외자 이름 오발송 위험 수정**. `kakao_send.py`의 방 열림 검증(`room_opened`)·잔류창 정리(`close_rooms`)가 제목을 **단순 포함/전방일치**로 비교해, 검색어가 다른 학생 방 이름의 접두일 때(예: "오직 이건" 검색 시 "오직 이건호" 방 제목도 통과) 오탐 가능 — 카톡 자체 검색이 엉뚱한 방을 열어도 코드가 이를 못 걸러 **오발송 위험**. `_title_matches_room()` 신설(경계 인식 매칭: 방 이름 뒤로 인원수 "(3)" 등 비-이름 문자만 허용, 한글/영숫자로 이어지면 거부) 도입, `room_opened`·`close_rooms` 양쪽에 적용. 검색·전송 로직 자체는 불변(검증 게이트만 강화). |
| 9.07 | 2026-08-07 | **[에이전트 0.96] OpenAI Responses A/B 빌드** — `DRW-AI-Agent-0.96.exe`를 별도 산출해 기존 0.94·0.95 파일을 보존한다. 에이전트 UI 버전, 빌드 스크립트 산출물명, 웹 다운로드 링크(`AGENT_DL`·guide), `app-report.js` cache-bust를 0.96으로 동기화. |
| 9.06 | 2026-08-07 | **[에이전트] OpenAI 자연어 품질 A/B 기반 이행** — OpenAI 선택 경로를 `gpt-4o-mini` Chat Completions에서 **`gpt-5.6-terra` Responses API**로 교체. 단건은 `reasoning.effort=none`으로 짧은 학부모 리포트의 불필요한 추론 토큰·지연을 억제하고, 기존 프롬프트의 최근 이력·문체·규칙으로 다양성을 유지한다. 일괄 생성은 `BATCH_RESPONSE_SCHEMA` strict Structured Outputs로 `{cls,name,note}` JSON 배열을 강제해 코드블록/서두·후미 산문에 의존하던 후처리 실패를 예방한다. Gemini·Claude·기본 엔진 설정은 불변 — 강사가 OpenAI를 선택한 경우에만 A/B 적용. |
| 9.08 | 2026-08-13 | **[에이전트] OpenAI 엔진 안정화 업데이트(9.06/9.07 Responses 시도 재롤백) — gpt-4o-mini → gpt-5.6-luna + gemini 키테스트 오판 수정**. ①OpenAI: gpt-4o-mini는 API 자체 은퇴 공지 없으나 세대가 오래됨 → GA·저가형 공식 후속 `gpt-5.6-luna`로 전환. Chat Completions 엔드포인트 그대로 유지(Responses API 미전환, 최소 변경 리스크). 파라미터: `max_tokens`→`max_completion_tokens`, `temperature` 제거(미지원), `reasoning_effort:"none"` 신규(불필요한 추론 억제, gemini/claude와 동일 취지). 실측 비교(4시나리오×2회): 4o-mini 평균 129자(목표 100자 잦은 초과·"오늘 수업에서는"·"~기대합니다" 상투구 반복) vs luna 평균 91자(분량 안정·구체적 서술), 하우스룰 위반은 둘 다 0. 단건·배치·키테스트 3경로 실콜 검증 통과. ②Gemini 키 테스트 간헐 오류 수정: 검증 프로브 8→64토큰(flash-latest thinking이 초소형 예산 먹고 MAX_TOKENS·텍스트없음으로 끝나 유효키 오판하던 원인) + HTTP 200 도달 후 텍스트없음/MAX_TOKENS는 인증 성공(키 유효)으로 판정 — 10회 연속 안정 확인(이전 간헐 실패). ③9.06/9.07의 gpt-5.6-terra Responses API 전환은 배치 스키마 루트 array(400 필수 오류)·키검증 토큰부족(400 오판) 실콜 확정 후 Chat Completions 구성으로 롤백. 에이전트 0.96(테스트) 빌드. |
| 9.07 | 2026-08-07 | **[에이전트 0.96] OpenAI Responses A/B 빌드 — gpt-4o-mini → gpt-5.6-luna + gemini 키테스트 오판 수정**. ①OpenAI: gpt-4o-mini는 API 자체 은퇴 공지 없으나 세대가 오래됨 → GA·저가형 공식 후속 `gpt-5.6-luna`로 전환. Chat Completions 엔드포인트 그대로 유지(Responses API 미전환, 최소 변경 리스크). 파라미터: `max_tokens`→`max_completion_tokens`, `temperature` 제거(미지원), `reasoning_effort:"none"` 신규(불필요한 추론 억제, gemini/claude와 동일 취지). 실측 비교(4시나리오×2회): 4o-mini 평균 129자(목표 100자 잦은 초과·"오늘 수업에서는"·"~기대합니다" 상투구 반복) vs luna 평균 91자(분량 안정·구체적 서술), 하우스룰 위반은 둘 다 0. 단건·배치·키테스트 3경로 실콜 검증 통과. ②Gemini 키 테스트 간헐 오류 수정: 검증 프로브 8→64토큰(flash-latest thinking이 초소형 예산 먹고 MAX_TOKENS·텍스트없음으로 끝나 유효키 오판하던 원인) + HTTP 200 도달 후 텍스트없음/MAX_TOKENS는 인증 성공(키 유효)으로 판정 — 10회 연속 안정 확인(이전 간헐 실패). ③codex의 OpenAI Responses API(gpt-5.6-terra) 전환 시도는 배치 스키마 루트 array(400 필수 오류)·키검증 토큰부족(400 오판) 실콜 확정 후 Chat Completions 구성으로 롤백. 에이전트 0.96(테스트) 빌드. |
| 9.05 | 2026-07-24 | **[에이전트] Gemini 지속 503 근본 대응 — gemini-flash-latest 전환 + 방어 2겹**. gemini-3.5-flash가 무료 트래픽 상시 혼잡으로 **지속 503(high demand)** — 같은 키·시각 다른 모델(2.5-flash·flash-latest·3.5-lite)은 전부 200으로, 키·파라미터 아닌 **단일 모델 혼잡**이 원인임을 실콜로 확정. 후보 실생성 품질 비교(자연어 리포트 관점): 3.5-lite는 "오늘의 OOO" 헤더를 본문에 반복(규칙 위반, 3회 재현)·과다길이, flash-latest(+thinkingLevel minimal)는 113~117자·헤더 미중복·태그 완곡 반영으로 **최상** → `GEMINI_MODEL=gemini-flash-latest`(구글 관리 이동 별칭, 모델 혼잡 자동 회피). 별칭 이동 리스크 방어: ①`_call_ai_hub` gemini 400+'thinking' 시 thinkingConfig 제거 후 1회 자동 재시도(추가 시도 미소비) ②응답 파서 하드닝 — parts 없는 후보(thinking-only·MAX_TOKENS·안전필터)에 원시 KeyError 대신 finishReason 포함 메시지·멀티파트 안전 합치기. 키 테스트 5xx 분류(9.04)와 결합해 503=키 무효 아님 안내. 에이전트 0.95 재빌드·clobber 교체. |
| 9.04 | 2026-07-22 | **[에이전트 0.95] API 키 유효성 검증 수단 추가**. 설정창 API 키 입력칸 아래 **「🔑 키 테스트」 버튼 + 결과 라벨** 신설 — 저장 전 키 상태 즉시 진단. `ai_engine.validate_key(engine, key)`: 초소형 실콜 1회(8토큰·**재시도 없음**, 스레드 실행으로 UI 무정지)로 4분류 — ✅유효 / ❌인증실패(401·403) / ⚠️쿼터제한(429; 키는 유효·무료 한도 소진, KST 오후 4~5시 리셋 안내) / ❌모델접근불가(404). 이를 위해 `_call_ai_hub`에 `retries` 파라미터 추가(기본 4로 **기존 생성 경로 동작 불변**, 검증만 1회). 실키 검증: gemini·claude ✅, 가짜키 ❌400, 빈키 안내. AGENT_VERSION·산출물명·웹 DL 링크(guide·app-report)·캐시버스트 v326 동기화. |
| 9.03 | 2026-07-22 | **[에이전트] Gemini gemini-3.5-flash 유지 확정 — 무료 실콜 검증**. 실사용 중 단일 생성에서 429 quota exceeded 발생 → 2.5-flash 롤백 검토했으나, 저장 키로 실콜 전수 검증 결과 **3.5-flash 정상**(단일·6연속 버스트·프로덕션 구성 520tok 전부 200, thinkingLevel minimal에서 thoughts=0·정상 문장). 당시 429는 순간 분당/일일 쿼터 창으로 판단(무료 일일 쿼터는 태평양 자정=KST 오후 4~5시 리셋, 키별 쿼터 상이). 역방향 확인: **2.5 계열은 신규 사용자 차단 진행 중**(2.5-flash-lite 404 "no longer available to new users") → 2.5 회귀 시 신규 강사 키에서 파손 위험이라 회귀 금지. lite 폴백 체인은 불채택(사용자 판단: 동일 쿼터 오류 예상). 순간 429는 기존 `_RETRY` 백오프(1·2·4s)가 흡수. 코드 변경 없음(9.01 구성 그대로), 주석·문서만 갱신. |
...
| 8.91 | 2026-06-26 | **빈 항목 메시지 제외 — 진도/과제/수행도 미입력 시 헤더째 생략**. `_buildMessage`(app-report.js, 최종 발송문+미리보기)가 내용이 비어도 `▶ 오늘의 진도` 등 헤더를 항상 출력하던 것 → 블록 배열로 재구성해 `progress`·`homework`·`assignLines`가 비면 그 `▶` 블록 자체를 생략. 특이사항(`▶ 오늘의 OO?`)은 핵심 코멘트라 유지. 발송 제외(`_excludeProg`)와도 자연 호환(제외 시 빈값→블록 생략). 웹 v320. |
| 8.90 | 2026-06-26 | **결석 버튼을 si-left(학생 식별 영역)로 이동 — 출결=학생 레벨 상태**. 과제 행에 있던 결석을 신호등 dot·이름과 같은 좌측 열(이름 아래)로. 근거: 결석은 '과제'가 아닌 **출결**(학생당 첫 게이트 판단)이고, 좌측 열은 하드차단 dim존(`.si-right .tg-cell`) 밖이라 카드 차단 시에도 결석 토글만 또렷(해제 affordance). 결석이 tg-cell을 떠나 dim 규칙 단순화(`:not([data-p])` 예외 제거). 결석 시 이름 빨강+취소선. `.tg-absent` 좌측열 맞춤(width:100%). 웹 v319·CSS v20260641. |
...
- v2.5.1 메시지 이름: 이름 끝의 식별용 연속 숫자만 제거한 뒤 기존 성 제외·받침 조사 규칙 적용. `이도현8`/`이도현12` → `도현이는`, `김민수3` → `민수는`. 일괄 공지 `{이름}`은 성을 포함한 정리된 이름 사용.
- 원본 학생명·nameKey·수신자 이름·카카오톡방 검색명·AI 일괄 응답 식별값은 보존. 본문·과제·날짜의 숫자는 제거하지 않음. 숫자만 있는 값은 빈 이름으로 만들지 않고 원본 유지.

전송 모니터 자동 정리: 최신 상태 ETag 조건부 저장으로 완료·취소·삭제 경합을 보호한다. 하트비트 조회 실패 시 오류 전환하지 않는다. 수신자 전원 `완료`인 `sending`/자동 정리 오류 작업만 `done`으로 복구하며, 실제 작업 오류·부분 완료는 보존한다.

일괄 공지는 텍스트만·이미지만·텍스트와 이미지 모두 전송할 수 있다. 메시지 공백은 미입력으로 취급하며, 텍스트와 이미지가 모두 없거나 수신자가 없으면 전송하지 않는다. 이미지 단독 전송은 빈 메시지와 이미지 첨부를 큐에 전달한다.

이미지 포함 전송은 단독/혼합 여부와 관계없이 채팅방을 자동으로 닫지 않는다. 이미지 전송 확인 후 다음 수신자로 진행하며 전체 종료 시 업로드 대기나 창 정리를 수행하지 않는다. 전송 확인은 카카오톡 UI 기준이며 서버 업로드 완료를 보증하지 않는다. 텍스트 전용 방의 기존 정리는 유지한다.
...
| `gemini` | `gemini-flash-latest` | 무료·기본, 가용 Flash 모델 자동 라우팅 |
| `claude` | `claude-sonnet-5` | 문장력·감성 우선 |
| `openai` | `gpt-5.6-terra` (Responses API) | 자연어 품질 A/B · strict JSON 일괄 생성 |
...
| Gemini 모델 | `gemini-flash-latest` | `GEMINI_MODEL` (constants.py) |
| Claude 모델 | `claude-sonnet-5` | ai_engine.py |
| OpenAI 모델 | `gpt-5.6-terra` | `OPENAI_MODEL` (constants.py) |
