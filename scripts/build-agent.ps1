...
# 산출물: code/dist/DRW-AI-Agent-0.100.exe  (build/·dist/·*.spec 은 gitignore)
#
# ⚠️ 실사용 exe는 이 dist/ 밖 별도 고정 폴더로 복사해서 그쪽에서 실행할 것.
#    설정파일(agent_config.json)이 exe와 같은 폴더에 생성되는데, dist/에서 직접 실행하면
#    다음 -Clean 재빌드 때 그 설정까지 통째로 삭제됨(실사고 발생 이력 — 2026-08-18).
param([switch]$Clean)
...
Write-Host "`n[완료] code/dist/DRW-AI-Agent-0.100.exe" -ForegroundColor Green
